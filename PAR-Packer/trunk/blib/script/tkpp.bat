...
my %general_configuration = (
  authors => {
    DJIBEL   => [ 'Djibril Ousmanou', 'djibel@cpan.org',   undef ],
...
my %options_pp = (
  ...
  '--addlist' => {
    -comment =>
      'Read a list of file/directory names from file selected, adding them into the package. (--addlist)',
  },
  '--bundle' => {
    -value => 0,
...
$file_menu->command(
  ...
  -command => sub {
    if ( my $file
      = save_file( $main, [ 'Batch Files', ['.bat'] ], $general_configuration{commandline_path} ) )
    {
...
sub display_size_notebook {
  ...
  my $button_dependent = get_checkbutton_widget( $labframe_executable_size,
    { variable => \$options_pp{'--dependent'}{-value}, } );
  ...
  my $button_add_libraries = $labframe_filereplib->ColoredButton(
    ...
    -command => sub {
      foreach
        my $lib_file ( get_file( $labframe_filereplib, [ 'Shared Library Files', [ '.so', '.dll' ] ], 1 ) )
      {
    ...
    { text    => '...',
      ...
      command => sub {
        $options_pp{'--ppfile'}{-value}
          = get_file( $labframe_general,
          [ 'pp Batch File', [ basename( $general_configuration{pp_path} ), ] ] )
          || $options_pp{'--ppfile'}{-value};
        }
    }
  );
...
sub build_pp {
  ...
  if ( !-e $ref_options->{'--ppfile'}{-value} ) {
    my $error
      = 'The path to ' . basename( $general_configuration{pp_path} ) . ' has not been set or invalid.';
    popup_information_warning( $widget, $error );
  ...
  if ( -e $output ) {
    my $warning
      = 'The output ' . basename($output) . ' already exists.' . "\n" . 'Do you want overwrite it ?';
  ...
  execute_command( $EXECUTABLE_NAME, $command );

  return;
}

sub execute_command {
  my ( $executable, $arguments ) = @_;
  ...
  if ( $OSNAME eq 'MSWin32' ) {
    ...
    Win32::Process::Create( $win32_process_buiding, $executable, $arguments, 0, $flags, '.' )
      or print_error( Win32::FormatMessage( Win32::GetLastError() ) );

    $build_button->configure( -state => 'disabled' );
    $build_button->redraw_button;

    # Check process id running and enabled Build button
    $main->after( $REPEAT_FILE_TIME, [ \&check_process_id, $logfile, \$octet_size_read ] );
...
sub check_process_id {
  my ( $logfile, $ref_octet_size_read ) = @_;
  ...
  if ( $exitcode == STILL_ACTIVE() ) {
    $main->after( $REPEAT_FILE_TIME, [ \&check_process_id, $logfile, $ref_octet_size_read ] );
    return;
  }
  ...
  if ( my $error = Win32::GetLastError() ) {
...
Copyright 2003, 2004, 2005, 2006, 2011, 2014 by Doug Gruber E<lt>doug(a)dougthug.comE<gt>,
